...
export default function Video() {
  ...
  async function like() {
    try {
      ...
      // Fetch updated video info after like
  ...
  async function dislike() {
    try {
      ...
      // Fetch updated video info after dislike
      const updatedRes = await fetch(`http://localhost:5000/api/video/info/${videoId}`);
      const updatedData = await updatedRes.json();
      setVideoData(updatedData);
    } catch (error) {
    ...
    <>
      {videoData ? (
        <div className="p-10 max-w-screen-md sm:mx-auto mx-10">
          ...
          {/* Video description with "Read More"/"Read Less" toggle */}
          ...
          <div className="flex gap-6 mt-4 text-sm">
            ...
            <button
              className="flex text-xl items-center gap-1 cursor-pointer"
              onClick={like}
            >
              <AiFillLike className="hover:scale-110 text-2xl text-blue-500" />
              {videoData.likes}
            </button>
            ...
            <button
              ...
              onClick={dislike}
            >
              <AiFillDislike className="hover:scale-110 text-2xl text-red-500" />
              {videoData.dislikes}
            </button>
          </div>
